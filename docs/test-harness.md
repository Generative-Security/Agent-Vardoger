...
Jump to: [Using it](#using-it) · [Teardown](#teardown) · [If something does not work](#if-something-does-not-work) · [Choosing a model](#choosing-a-model)
...
The harness does nothing but call a tool, so the model's tool-use behaviour
is the whole test. Each note below is a failure that looks like a broken
